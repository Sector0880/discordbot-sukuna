					...
					if guild_language(ctx = message) == "ru": await message.channel.send("Быстрый доступ разрабатывается, просим немного подождать.")
						...
						await message.channel.send(embed = discord.Embed(
							description = "\n".join([
								...
								error_invalid_language()["ru"]["error"]["description2"].format(", ".join(languages))
							]),
							color = color_error
						))
						return await message.channel.send(embed = discord.Embed(
							description = "\n".join([
								...
								error_invalid_language()["uk"]["error"]["description2"].format(", ".join(languages))
							]),
							color = color_error
						))
		except:
