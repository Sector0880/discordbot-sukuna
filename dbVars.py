...
bot_presence = lambda: yaml.safe_load(open(botConfig.path_db_bot, 'r', encoding='utf-8'))['presence']

sf_c = lambda: yaml.safe_load(open(botConfig.path_db_staff, 'r', encoding='utf-8'))['staffList_Creators']
sf_a = lambda: yaml.safe_load(open(botConfig.path_db_staff, 'r', encoding='utf-8'))['staffList_Admins']
sf_sp = lambda: yaml.safe_load(open(botConfig.path_db_staff, 'r', encoding='utf-8'))['staffList_SpecialPerms']

cspl_initial_guilds = lambda: yaml.safe_load(open(botConfig.path_db_cspl_initial_guilds, 'r', encoding='utf-8'))
cspl_initial_users = lambda: yaml.safe_load(open(botConfig.path_db_cspl_initial_users, 'r', encoding='utf-8'))

cspl_custom_guilds = lambda interaction: json.load(open(botConfig.path_db_cspl_custom_guilds, "r", encoding="utf-8"))
cspl_custom_users = lambda interaction: json.load(open(botConfig.path_db_cspl_custom_users, "r", encoding="utf-8"))

# берет объект из базы данных, если в json он существует то он берет именно его из json
def cspl_get_param(interaction, branch: str, param: str, path: list = None, _user: discord.Member = None, guild = None):
	try:
		...
		match branch:
			case 'g':
				if path and path_len == 4:
					if (
						guild_id in cspl_custom_guilds(interaction).keys()
						and path[0] in cspl_custom_guilds(interaction)[guild_id].keys()
						and path[1] in cspl_custom_guilds(interaction)[guild_id][path[0]].keys()
						and path[2] in cspl_custom_guilds(interaction)[guild_id][path[0]][path[1]].keys()
						and path[3] in cspl_custom_guilds(interaction)[guild_id][path[0]][path[1]][path[2]].keys()
				...
				if path and path_len == 3:
					if (
						...
						and path[1] in cspl_custom_guilds(interaction)[guild_id][path[0]].keys()
						and path[2] in cspl_custom_guilds(interaction)[guild_id][path[0]][path[1]].keys()
						and param in cspl_custom_guilds(interaction)[guild_id][path[0]][path[1]][path[2]].keys()
					):
				...
				if path and path_len == 2:
					if (
						...
						and path[0] in cspl_custom_guilds(interaction)[guild_id].keys()
						and path[1] in cspl_custom_guilds(interaction)[guild_id][path[0]].keys()
						and param in cspl_custom_guilds(interaction)[guild_id][path[0]][path[1]].keys()
					):
					...
					if (
						guild_id in cspl_custom_guilds(interaction).keys()
						and path[0] in cspl_custom_guilds(interaction)[guild_id].keys()
						and param in cspl_custom_guilds(interaction)[guild_id][path[0]].keys()
					):
					...
					if (
						guild_id in cspl_custom_guilds(interaction).keys()
						and param in cspl_custom_guilds(interaction)[guild_id].keys()
					):
			...
			case 'u':
				...
				if path and path_len == 2:
					if (
						user_id in cspl_custom_users(interaction).keys()
						and guild_id in cspl_custom_users(interaction)[user_id].keys()
						and path[0] in cspl_custom_users(interaction)[user_id][guild_id].keys()
						and path[1] in cspl_custom_users(interaction)[user_id][guild_id][path[0]].keys()
						and param in cspl_custom_users(interaction)[user_id][guild_id][path[0]][path[1]].keys()
					):
					...
					if (
						...
						and guild_id in cspl_custom_users(interaction)[user_id].keys()
						and path[0] in cspl_custom_users(interaction)[user_id][guild_id].keys()
						and param in cspl_custom_users(interaction)[user_id][guild_id][path[0]].keys()
					):
					...
					if (
						user_id in cspl_custom_users(interaction).keys()
						and guild_id in cspl_custom_users(interaction)[user_id].keys()
						and param in cspl_custom_users(interaction)[user_id][guild_id].keys()
					):
